...
def find_torrent_list(search_string):
    ...
    for i in soup.find_all('a', class_='detLink'):
        if 'Details for' in i['title']:
            ...
            list_of_titles.append(title_string[1])

    for link in soup.find_all('a'):
        i = link.get('href')
        if 'magnet' in i:
            list_of_magnets.append(i)
    ...
    for timestamp in soup.find_all('font', class_='detDesc'):
        timestamp = str(timestamp).split(',')[0].split('Uploaded')[1]
        timestamp = timestamp.replace(u'\xa0', u' ')
        list_of_timestamps.append(timestamp)

    return list_of_titles, list_of_magnets, list_of_sizes, list_of_timestamps
